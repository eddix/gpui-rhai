...
| F. Events/refs/focus/scroll/a11y | Missing | one handler per semantic name; direct Host callback; basic GPUI click/key/focus/scroll mechanisms in legacy renderers | normalized raw events; capture/target/bubble lists; prevention and pointer capture; `NativeHandlerRef`; `ElementRef`; geometry; nested scroll router; public focus/layer behavior; retained accessibility tree |
