...
| D. Atomic visual surface | Partial | final `Box` node kind; nested layout-transparent `Fragment` flattening with decoration rejection; row/column/stack Box helpers; plain Text plus typed inline Span runs rendered by one GPUI `StyledText`; current image/style subset | full wrapping/alignment/ellipsis/clamp/link/selection/search text contract; svg/canvas atoms; exhaustive typed layout/paint; font system; namespaced themes; Canvas retained scene |
