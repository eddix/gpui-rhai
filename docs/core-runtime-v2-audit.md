...
| F. Events/refs/focus/scroll/a11y | Partial | ordered capture/target/bubble binding model; independent response controls; real GPUI pointer down/up/move/wheel normalization; direct Host callback and schema-checked `NativeHandlerRef` through one outer transaction; retained `ElementRef` to `NodeId`; stale/key/duplicate validation; prepaint geometry and exact read invalidation | transformed local/content coordinates; central retained hit path; actual pointer capture/release routing and continuous-event coalescing; frame accounting; focus/scroll/ref commands; nested scroll router; public layer behavior; retained accessibility tree |
