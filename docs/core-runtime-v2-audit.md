...
| I. Tooling/apps | Partial | manifest/component checks, Inspector baseline, current examples; `artistic_showcase` pure-Rhai application exercises Box/Fragment/Span/Canvas/NativeSignal/raw pointer/theme atoms; pure-Rhai `mini_timeline` uses retained Canvas, relative/absolute Stack, NativeSignal playhead, raw drag and pointer capture and has a prepare gate | screenshot and 120 Hz interaction certification for showcase/Timeline; final schema/definition emission and LSP; automation IDs/query/actions/screenshots; variable-height chat; Rust fast-path Timeline parity app |
