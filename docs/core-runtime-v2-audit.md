...
| D. Atomic visual surface | Partial | final `Box` node kind; nested layout-transparent `Fragment` flattening with decoration rejection; row/column/stack Box helpers; current text/image/style subset | span/svg/canvas atoms; exhaustive typed layout/paint/text; font system; namespaced themes; rich text/selection/search; Canvas retained scene |
