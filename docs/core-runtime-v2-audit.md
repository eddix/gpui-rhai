...
| D. Atomic visual surface | Partial | final `Box` node kind; nested layout-transparent `Fragment` flattening with decoration rejection; row/column/stack Box helpers; plain Text plus typed inline Span runs rendered by one GPUI `StyledText`; retained keyed Canvas scene with validated rect/circle/line commands, typed colors, paint-only Rust execution, and command budget; current image/style subset | full wrapping/alignment/ellipsis/clamp/link/selection/search text contract; svg atom; exhaustive typed layout/paint; font system; namespaced themes; Canvas path/gradient/transform/clip/hit/events/a11y/signals |
...
| G. Native mechanisms | Partial | mount/render/unmount custom primitive registry; variable-height GPUI `list/ListState` production element; deterministic Fenwick measurement/overdraw/anchor/reorder/tail core with 10k-item tests; native text input/area | primitive update lifecycle and retained Entity/resource ownership; data-backed formal item execution is still eager; follow-tail/measurement metrics wiring and no-empty-frame benchmark; text undo/redo and document infrastructure |
