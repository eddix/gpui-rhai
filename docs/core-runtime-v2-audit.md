...
| G. Native mechanisms | Partial | keyed custom primitive mount/update(previous,next)/render/unmount with retained normalized snapshots and panic boundaries; data-backed `virtual_collection` stores UiValue, validates keys, executes only the initial estimated window, queues GPUI layout requests without Rhai, realizes named/formal item components on the next foreground transaction, replaces offscreen state/effects, and preserves GPUI `ListState` measurements; deterministic Fenwick measurement/anchor/tail core with 10k tests; native text input/area | delete the old eager `virtual_list` path with privileged Table/Dropdown; explicit task/subscription/capture/resource ownership on primitive instances and Entity-cycle gate; expose measurement/window metrics and complete no-empty-frame/follow-tail benchmark; text undo/redo and document infrastructure |
