...
| I. Tooling/apps | Partial | manifest/component checks, Inspector baseline, current examples; `artistic_showcase` pure-Rhai application exercises Box/Fragment/Span/Canvas/NativeSignal/raw pointer/theme atoms; pure-Rhai `mini_timeline` uses retained Canvas, relative/absolute Stack, NativeSignal playhead, raw drag and pointer capture and has a prepare gate; `variable_height_chat` renders 2,000 varied keyed items through the production variable-list contract in an automated source/render test | screenshot and 120 Hz interaction certification for showcase/Timeline/chat; final schema/definition emission and LSP; automation IDs/query/actions/screenshots; chat must migrate from eager item snapshots to data-backed formal item execution; Rust fast-path Timeline parity app |
