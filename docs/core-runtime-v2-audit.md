...
| I. Tooling/apps | Partial | manifest/component checks, Inspector baseline, current examples; `artistic_showcase` pure-Rhai application exercises Box/Fragment/Span/Canvas/NativeSignal/raw pointer/theme atoms | screenshot and performance certification for artistic showcase; final schema/definition emission and LSP; automation IDs/query/actions/screenshots; pure-Rhai Mini Timeline; variable-height chat; Rust fast-path parity app |
