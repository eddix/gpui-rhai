...
DatePicker is a controlled, single-date form control backed by a keyed native
calendar entity. It presents a read-only Input-like trigger and an anchored
month panel. The public date ABI is strict Gregorian ISO `YYYY-MM-DD`; locale
...
- `disabled`, `error`, `size`, and `placement`;
...
Open state, visible month, focused day, and hover state belong to the native
entity. DatePicker does not expose controlled `open` or `visible_month` props.
...
## Native and locale boundary

The Rhai component owns props, size metrics, style, parts, and asset IDs. Rust
owns Gregorian arithmetic, focus, keyboard navigation, overlay coordination,
and the keyed entity. It reuses the shared window overlay coordinator rather
than implementing placement or dismissal again.
...
- native keyboard, dismissal, focus restoration, and controlled-value tests;
