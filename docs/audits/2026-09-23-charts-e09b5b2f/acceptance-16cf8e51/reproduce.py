...
def main():
    ...
    parser.add_argument("mode", choices=("public", "native", "linked"))
    ...
    manifest = (
        f'[package]\nname="chart-acceptance-{package}-16cf8e51"\nversion="0.0.0"\nedition="2024"\n'
        '[workspace]\n[dependencies]\n'
    ...
    with tempfile.TemporaryDirectory(prefix=f"chart-acceptance-{args.mode}-") as work:
        ...
        if args.mode == "native":
            command += ["--", "--nocapture", "--skip", "idle_linked_charts_do_not_repeat_layout"]
        elif args.mode == "linked":
            command += ["idle_linked_charts_do_not_repeat_layout", "--", "--nocapture"]
