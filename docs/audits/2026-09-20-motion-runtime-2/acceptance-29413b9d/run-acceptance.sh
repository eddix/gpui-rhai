...
probe_dir="$(mktemp -d "${TMPDIR:-/tmp}/gpui-rhai-motion-acceptance.XXXXXX")"
mkdir -p "${probe_dir}/src"
cp "${audit_dir}/acceptance.rs" "${probe_dir}/src/main.rs"
...
(p/'Cargo.toml').write_text('[package]\nname="gpui-rhai-motion-acceptance-29413b9d"\nversion="0.0.0"\nedition="2024"\n[workspace]\n[dependencies]\ngpui-rhai={path='+json.dumps(str(root/'crates/gpui-rhai'))+',features=["dev-reload","grain-backend"]}\n')
PY
