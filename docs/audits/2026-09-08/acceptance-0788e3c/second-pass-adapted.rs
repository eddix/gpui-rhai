...
fn main() {
    ...
    let geometry_a = shared.borrow_mut().presentation_geometry("a");
    let geometry_b = shared.borrow_mut().presentation_geometry("b");
    let capture_a = shared.borrow_mut().presentation_pointer_capture("a");
    ...
    println!(
        ...
        ra == rb,
        geometry_a.read(child, &pa).is_err(),
        capture_a.captured(1).is_none()
    );
    geometry_a.update(
        ra,
        ElementGeometry {
    ...
    println!(
        ...
        geometry_a.read(ra, &pa).unwrap().visual.width
    );
