...
def main():
    ...
    parser.add_argument("mode", choices=("native", "previous-native"))
    ...
    if args.mode == "previous-native":
        previous = repo / "docs/audits/2026-09-23-charts-e09b5b2f/acceptance-42afce01/reproduce.py"
        return subprocess.run([
            sys.executable, str(previous), "native",
            "--out-dir", str(out),
        ], cwd=repo).returncode

    manifest = (
        '[package]\nname="ui-theme-native-e5d26877"\nversion="0.0.0"\nedition="2024"\n'
        '[workspace]\n[dependencies]\n'
