...
fn validate_fonts(root: &Path) -> Result<(), ProjectError> {
    let files = collect_relative_files(&root.join("ui/fonts"), "../ui/fonts")?;
    let mut fonts = Vec::with_capacity(files.len());
    for (name, _) in files {
        validate_font_extension(&name)?;
        let path = root.join("ui/fonts").join(&name);
        let bytes = fs::read(&path).map_err(|source| ProjectError::Io {
            path: path.clone(),
            source,
        })?;
        fonts.push(
            FontSource::new(name, bytes).map_err(|error| ProjectError::Font(error.to_string()))?,
        );
    }
    validate_font_sources(&fonts).map_err(|error| ProjectError::Font(error.to_string()))
}
...
fn generated_embed_module(
    ...
    output.push_str("];\n\n");
    output.push_str("pub fn font_sources() -> Vec<gpui_rhai::FontSource> {\n    vec![\n");
    for (name, path) in fonts {
        let _ = writeln!(
            output,
            "        gpui_rhai::FontSource::new({name:?}, include_bytes!({path:?}).to_vec()).expect(\"generated font source\"),"
        );
    }
    output.push_str("    ]\n}\n");
    Ok(output)
...
fn validate_font_extension(path: &str) -> Result<(), ProjectError> {
    match path.rsplit('.').next().unwrap_or_default() {
        "ttf" | "otf" | "ttc" => Ok(()),
        _ => Err(ProjectError::UnsupportedFont(path.to_owned())),
    }
}
...
pub enum ProjectError {
    ...
    #[error("font `{0}` has an unsupported extension; expected ttf, otf, or ttc")]
...
mod tests {
    ...
    fn embed_plan_contains_entry_components_theme_locale_and_assets() {
        ...
        fs::create_dir_all(directory.path().join("ui/fonts")).unwrap();
        fs::write(directory.path().join("ui/locales/en.rhai"), "#{}\n").unwrap();
        fs::write(directory.path().join("ui/assets/check.svg"), "<svg/>\n").unwrap();
        fs::write(
            directory.path().join("ui/fonts/art.otf"),
            [b"OTTO".as_slice(), &[0, 1, 2]].concat(),
        )
        .unwrap();
        ...
        assert!(generated.contains("pub fn font_sources()"));
